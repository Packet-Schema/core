...
describe("JSON Schema — psdl-0.5.yaml", () => {
  ...
  it("fixes the RfcRef acceptance boundary (§5.4)", () => {
    ...
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [1.5] } }, body: [] })).toBe(false);
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: ["x"] } }, body: [] })).toBe(false);
    // Unknown provenance keys are rejected (additionalProperties: false).
    expect(validate({ name: "t", meta: { rfc: { defined: 791, obsoletes: [760] } }, body: [] })).toBe(false);
    // defined must be an integer, not a numeric string.
    expect(validate({ name: "t", meta: { rfc: { defined: "791" } }, body: [] })).toBe(false);
  });

  it("accepts object-form updates entries with an optional section (§5.4, D8)", () => {
    // Object-form { rfc, section? } entry.
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [{ rfc: 2474, section: "3" }] } }, body: [] })).toBe(true);
    // Bare + object mix.
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [2474, { rfc: 3168, section: "5" }] } }, body: [] })).toBe(true);
    // section is optional in the object form.
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [{ rfc: 2474 }] } }, body: [] })).toBe(true);
  });

  it("rejects malformed object-form updates entries (§5.4, D8)", () => {
    // Object form missing the required `rfc`.
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [{ section: "3" }] } }, body: [] })).toBe(false);
    // Surplus key in the object form.
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [{ rfc: 2474, foo: 1 }] } }, body: [] })).toBe(false);
    // rfc must be an integer.
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [{ rfc: "2474" }] } }, body: [] })).toBe(false);
    // section must be a string.
    expect(validate({ name: "t", meta: { rfc: { defined: 791, updates: [{ rfc: 2474, section: 3 }] } }, body: [] })).toBe(false);
  });
  ...
  it("accepts meta on bounded and encrypted regions (§5.4)", () => {
    const pkt = {
      ...
      body: [
        {
          kind: "bounded", id: "b", bytes: { kind: "lit", value: 4 },
          meta: { rfc: 9000 },
  ...
  it("accepts a field with a values dictionary (ValueEntry, §5.3)", () => {
    const pkt = {
      ...
      body: [{
        id: "dscp", name: "DSCP", type: { kind: "int", bits: 6 },
        values: [
          { value: 46, name: "EF", label: "Expedited Forwarding", level: "must", meta: { rfc: 3246 } },
  ...
  it("rejects a ValueEntry setting both value and range (oneOf)", () => {
    const pkt = {
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, values: [{ value: 1, range: [2, 3] }] }],
    };
  ...
  it("rejects a non-integer ValueEntry value (type: integer)", () => {
    const pkt = {
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, values: [{ value: 4.6 }] }],
    };
    expect(validate(pkt)).toBe(false);
  });
  ...
  it("rejects a ValueEntry range with the wrong arity (minItems/maxItems: 2)", () => {
    const entry = (range: number[]): unknown => ({
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, values: [{ range }] }],
    });
  ...
  it("rejects non-string ValueEntry name/label/doc (type: string)", () => {
    const entry = (extra: Record<string, unknown>): unknown => ({
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 16 }, values: [{ value: 404, ...extra }] }],
    });
  ...
  it("rejects an invalid ValueEntry level", () => {
    const pkt = {
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, values: [{ value: 1, level: "required" }] }],
    };
    expect(validate(pkt)).toBe(false);
  });
  ...
  it("accepts a ValueEntry pattern bit-predicate entry (§5.3)", () => {
    const pkt = {
      name: "t",
      body: [{ id: "dscp", name: "DSCP", type: { kind: "int", bits: 6 }, values: [{ pattern: "xxxx11", name: "EXP" }] }],
    };
    const ok = validate(pkt);
    if (!ok) console.error(validate.errors);
    expect(ok).toBe(true);
  ...
  it("rejects a ValueEntry pattern with illegal characters", () => {
    const pkt = {
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, values: [{ pattern: "xx12" }] }],
    };
    expect(validate(pkt)).toBe(false);
  });
  ...
  it("rejects a ValueEntry mixing value and pattern (oneOf)", () => {
    const pkt = {
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, values: [{ value: 1, pattern: "xx11" }] }],
    };
    expect(validate(pkt)).toBe(false);
  });
  ...
  it("accepts an enum variant object carrying level and meta (§3)", () => {
    const pkt = {
      ...
      body: [{
        id: "p", name: "P",
        type: { kind: "enum", bits: 8, variants: { "6": { label: "TCP", level: "may", meta: { rfc: 793 } } } },
      }],
    };
    const ok = validate(pkt);
    if (!ok) console.error(validate.errors);
  ...
  it("rejects a non-decimal enum variant key (propertyNames, §3)", () => {
    const pkt = {
      name: "t",
      body: [{ id: "p", name: "P", type: { kind: "enum", bits: 8, variants: { "0x6": { label: "TCP" } } } }],
    };
    expect(validate(pkt)).toBe(false);
  });
  ...
  it("rejects a non-string enum variant doc (type: string)", () => {
    const pkt = {
      name: "t",
      body: [{ id: "p", name: "P", type: { kind: "enum", bits: 8, variants: { "6": { label: "TCP", doc: 793 } } } }],
    };
    expect(validate(pkt)).toBe(false);
  });
  ...
  it("accepts meta on a defs struct and validates its shape (NamedStruct, §5.4/§6)", () => {
    ...
    expect(validate(defsPkt({ rfc: { defined: 791, updates: [2474] }, section: "3.1" }))).toBe(true);
  ...
  it("accepts a constraint with level: should and rejects an invalid level (§9.1)", () => {
    ...
    expect(validate({
      ...base,
      constraints: [{ lhs: { kind: "ref", field: "a" }, rhs: { kind: "lit", value: 1 }, level: "should" }],
    })).toBe(true);
    expect(validate({
      ...base,
      constraints: [{ lhs: { kind: "ref", field: "a" }, rhs: { kind: "lit", value: 1 }, level: "x" }],
    })).toBe(false);
  });
  ...
  it("rejects an unknown constraint key, e.g. a typo'd 'leval' (additionalProperties: false)", () => {
    expect(validate({
      name: "t",
      body: [{ id: "a", name: "A", type: { kind: "int", bits: 8 } }],
      constraints: [{ lhs: { kind: "ref", field: "a" }, rhs: { kind: "lit", value: 1 }, leval: "should" }],
    })).toBe(false);
  });
    ...
    expect(validate({
      ...
      body: [{
        ...
        checksumParams: {
          ...
          finalXOR: "0xFFFFFFFFFFFFFFFF", width: 64,
        },
      }],
    })).toBe(true);
  });
  ...
  it("rejects an unknown category token (closed CategoryToken enum, §5.1)", () => {
    expect(validate({
      name: "t",
      body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, category: "padding" }],
    })).toBe(false);
  });
  it("rejects an align container carrying a category (align has no category property)", () => {
    expect(validate({ name: "t", body: [{ kind: "align", to: 32, category: "reserved" }] })).toBe(false);
  });
});

describe("JSON Schema — bytes delimiter form (§3, D3)", () => {
  it("accepts the delimiter form", () => {
    expect(validate({ name: "t", body: [{ id: "rl", name: "RL", type: { kind: "bytes", n: { delimiter: [13, 10] } } }] })).toBe(true);
    expect(validate({ name: "t", body: [{ id: "fn", name: "FN", type: { kind: "bytes", n: { delimiter: [0] } } }] })).toBe(true);
  });
  it("still accepts the plain Expr length form", () => {
    expect(validate({ name: "t", body: [{ id: "d", name: "D", type: { kind: "bytes", n: { kind: "lit", value: 4 } } }] })).toBe(true);
    expect(validate({ name: "t", body: [{ id: "d", name: "D", type: { kind: "bytes", n: { kind: "remaining" } } }] })).toBe(true);
  });
  it("rejects an empty delimiter, an out-of-range byte, and an unknown key on the delimiter form", () => {
    expect(validate({ name: "t", body: [{ id: "x", name: "X", type: { kind: "bytes", n: { delimiter: [] } } }] })).toBe(false);
    expect(validate({ name: "t", body: [{ id: "x", name: "X", type: { kind: "bytes", n: { delimiter: [256] } } }] })).toBe(false);
    expect(validate({ name: "t", body: [{ id: "x", name: "X", type: { kind: "bytes", n: { delimiter: [0], consume: true } } }] })).toBe(false);
  });
});
...
describe("JSON Schema — subfields (§12, D4)", () => {
  it("accepts subfields with integer and hex-string masks", () => {
    expect(validate({
      ...
      body: [{
        ...
        subfields: [
          { id: "frameType", name: "Frame Type", mask: 7, category: "type", values: [{ value: 1, label: "Data" }] },
  ...
  it("rejects a subfield missing id/name/mask or with a surplus key", () => {
    expect(validate({ name: "t", body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, subfields: [{ id: "a", name: "A" }] }] })).toBe(false);
    expect(validate({ name: "t", body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, subfields: [{ id: "a", name: "A", mask: 1, bogus: 1 }] }] })).toBe(false);
  });
  it("rejects a non-0x mask string", () => {
    expect(validate({ name: "t", body: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, subfields: [{ id: "a", name: "A", mask: "ff" }] }] })).toBe(false);
  });
});
