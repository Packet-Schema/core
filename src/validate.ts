...
const SWITCH_KEY_RE =
  /^(_|(0|[1-9][0-9]*)|(0|[1-9][0-9]*)-(0|[1-9][0-9]*)|(0|[1-9][0-9]*)(,(0|[1-9][0-9]*))+)$/;
...
function isBytesDelimitedShape(n: unknown): boolean {
  return (
    typeof n === "object" && n !== null && !Array.isArray(n) && "delimiter" in n
  );
}
 ...
 */
const NON_CRC_CHECKSUM_ALGORITHMS: ReadonlySet<string> = new Set([
  "internet",
  "adler32",
]);
...
const ENUM_VARIANT_KEYS: ReadonlySet<string> = new Set([
  "label",
  "doc",
  "level",
  "meta",
...
const PACKET_META_KEYS: ReadonlySet<string> = new Set([
  "rfc",
  "section",
  "aliases",
  "tags",
...
function validateMeta(
  ...
  for (const key of Object.keys(meta)) {
    if (!allowedKeys.has(key))
      errors.push({
        message: `${ctx}: meta has unknown key "${key}" (allowed: ${[...allowedKeys].join(", ")}) (§5.4).`,
      });
  }
  ...
  if (rfc !== undefined && !isValidRfcRef(rfc))
    errors.push({
      message: `${ctx}: meta.rfc must be an integer or { defined, updates? } where each updates entry is an integer or { rfc, section? } (§5.4).`,
    });
  if (section !== undefined && typeof section !== "string")
    errors.push({
      message: `${ctx}: meta.section must be a string (e.g. "3.1") (§5.4).`,
    });
  if (
    allowedKeys.has("aliases") &&
    aliases !== undefined &&
    (!Array.isArray(aliases) || !aliases.every((a) => typeof a === "string"))
  )
    errors.push({
      message: `${ctx}: meta.aliases must be an array of strings.`,
    });
  ...
  if (
    allowedKeys.has("tags") &&
    tags !== undefined &&
    (!Array.isArray(tags) || !tags.every((t) => typeof t === "string"))
  )
    errors.push({
      message: `${ctx}: meta.tags must be an array of strings (§1.1).`,
    });
  if (
    allowedKeys.has("family") &&
    family !== undefined &&
...
export function isValidExpr(expr: unknown): expr is Expr {
  ...
  switch (e.kind) {
    case "lit":
      return (
        typeof (expr as { value?: unknown }).value === "number" &&
    ...
    case "op": {
      ...
      return (
        typeof o.op === "string" &&
        (BIN_OPS as readonly string[]).includes(o.op) &&
        isValidExpr(o.a) &&
        isValidExpr(o.b)
      );
    }
...
function validateExprPlacement(
  ...
  walkExpr(expr, (e) => {
    if (e.kind === "peek" && !PEEK_ALLOWED.has(slot))
      errors.push({
        message: `${ctx}: peek may not appear in ${slot} (allowed: switch.on, optional.when, repeat.count/until).`,
      });
    if (e.kind === "enclosingField" && slot !== "constraint")
      errors.push({
        message: `${ctx}: enclosingField may only appear in constraints, not ${slot}.`,
      });
    if (
      e.kind === "prevIter" &&
      slot !== "repeat.count" &&
      ...
      errors.push({
        message: `${ctx}: prevIter may only appear in repeat.count or repeat.until, not ${slot}.`,
      });
    ...
    if (
      pc.documentDeclaredIds !== undefined &&
      ...
      if (id.includes("#")) {
        errors.push({
          message: `${ctx}: '#'-qualified ids may not appear in expressions (repeat-indexed instances are not referenceable, §2/§10.4).`,
        });
        ...
        errors.push({
          message: `${ctx}: ${e.kind === "ref" ? "ref" : "wireSize"} target "${id}" is not declared anywhere in the document (§11.1).`,
        });
      }
    }
    ...
    if (slot !== "constraint") {
      if (e.kind === "remaining" && !pc.remainingOk)
        errors.push({
          message: `${ctx}: 'remaining' used outside a scope-providing container with a defined byte budget (§11.1).`,
        });
      if (e.kind === "enclosingBits" && !pc.enclosingBitsOk)
        errors.push({
          message: `${ctx}: 'enclosingBits' used outside a scope-providing container that carries an injected bit budget (§11.1).`,
        });
      // §10.1/§11.1: forward-reference rules for wireSize and repeat-id ref.
      if (e.kind === "wireSize") {
        if (pc.openIds.has(e.target))
          errors.push({
            message: `${ctx}: wireSize target "${e.target}" is an enclosing/not-yet-closed container (§11.1).`,
          });
        else if (!pc.declaredIds.has(e.target))
          errors.push({
            message: `${ctx}: wireSize target "${e.target}" does not precede this expression in document order (§11.1).`,
          });
      }
      ...
      if (
        e.kind === "ref" &&
        pc.declaredExprIds !== undefined &&
        pc.documentDeclaredIds !== undefined &&
        !pc.declaredExprIds.has(e.field) &&
        !e.field.includes("#") &&
        !(
          e.field.includes(".") &&
          pc.importPrefixes?.has(e.field.slice(0, e.field.indexOf(".")))
        )
      ) {
        if (pc.repeatIds.has(e.field) && !pc.declaredIds.has(e.field)) {
          errors.push({
            message: `${ctx}: ref to repeat container id "${e.field}" precedes that repeat in document order (§11.1).`,
          });
        } else if (
          pc.documentDeclaredIds.has(e.field) &&
          !elementRefExempt?.has(e.field)
        ) {
          ...
          errors.push({
            message: `${ctx}: ref target "${e.field}" does not precede this expression in document order, or refers to its own container (a self-size ref); a body expression may reference only fields declared before it (§10.1/§11.1).`,
          });
        }
      }
    }
  ...
  switch (type.kind) {
    case "int":
      if (!Number.isInteger(type.bits) || type.bits <= 0)
        errors.push({
          message: `${ctx}: int must have positive integer bits, got ${type.bits}.`,
        });
      return;
    case "enum": {
      if (!Number.isInteger(type.bits) || type.bits <= 0)
        errors.push({
          message: `${ctx}: enum must have positive integer bits, got ${type.bits}.`,
        });
      ...
      if (
        type.variants &&
        typeof type.variants === "object" &&
        !Array.isArray(type.variants)
      ) {
        for (const [k, v] of Object.entries(
          type.variants as Record<string, unknown>,
        )) {
          if (!DECIMAL_INT_RE.test(k))
            errors.push({
              message: `${ctx}: enum variant key "${k}" must be a non-negative decimal integer (§3).`,
            });
          if (typeof v === "object" && v !== null) {
            ...
            for (const key of Object.keys(o)) {
              ...
              if (!ENUM_VARIANT_KEYS.has(key))
                errors.push({
                  message: `${ctx}: enum variant "${k}" has unknown key "${key}" (allowed: ${[...ENUM_VARIANT_KEYS].join(", ")}) (§3).`,
                });
            }
            if (typeof o.label !== "string")
              errors.push({
                message: `${ctx}: enum variant "${k}" must have a string label (§3).`,
              });
            // Mirrors the schema's EnumVariant `doc` `type: string` (§3).
            if (o.doc !== undefined && typeof o.doc !== "string")
              errors.push({
                message: `${ctx}: enum variant "${k}" doc must be a string (§3).`,
              });
            if (o.level !== undefined && !NORM_LEVELS.has(o.level as string))
              errors.push({
                message: `${ctx}: enum variant "${k}" has invalid level "${String(o.level)}" (must be must|should|may).`,
              });
            validateMeta(o.meta, `${ctx}: enum variant "${k}"`, errors);
          } else if (typeof v !== "string") {
            errors.push({
              message: `${ctx}: enum variant "${k}" must be a string label or an object with a label (§3).`,
            });
          }
        }
      } else {
        ...
        errors.push({
          message: `${ctx}: enum must have a variants object (§3).`,
        });
      }
      return;
    }
    case "bits":
      if (!Number.isInteger(type.n) || type.n <= 0)
        errors.push({
          message: `${ctx}: bits must have positive integer n, got ${type.n}.`,
        });
      return;
    case "bytes":
      ...
      if (isBytesDelimitedShape(type.n)) {
        ...
        for (const key of Object.keys(type.n as Record<string, unknown>))
          if (key !== "delimiter")
            errors.push({
              message: `${ctx}: bytes.n delimiter form accepts only the "delimiter" key (got "${key}") (§3).`,
            });
        if (!Array.isArray(delim) || delim.length < 1)
          errors.push({
            message: `${ctx}: bytes.n delimiter must be a non-empty array of byte integers (§3/§11.1).`,
          });
        else if (
          !delim.every(
            (b) =>
          ...
          errors.push({
            message: `${ctx}: bytes.n delimiter elements must be integers in 0–255 (§3/§11.1).`,
          });
      } else if (!isValidExpr(type.n))
        errors.push({
          message: `${ctx}: bytes has a malformed length expression.`,
        });
      else validateExprPlacement(type.n, "bytes.n", ctx, errors, pc);
      return;
    case "varint":
      if (typeof type.encoding !== "string" || type.encoding.length === 0)
        errors.push({
          message: `${ctx}: varint encoding must be a non-empty string.`,
        });
      return;
    case "berLength":
      if (
        type.maxBytes !== undefined &&
        (!Number.isInteger(type.maxBytes) ||
          type.maxBytes < 1 ||
          type.maxBytes > 5)
      )
        errors.push({
          message: `${ctx}: berLength maxBytes must be 1–5, got ${type.maxBytes}.`,
        });
      return;
    default:
      errors.push({
        message: `${ctx}: unknown type kind "${(type as { kind: string }).kind}".`,
      });
  }
}
 ...
 * is not unique.
 */
function validateChecksumParams(
  field: Field,
  ctx: string,
  errors: ValidationError[],
): void {
  ...
  for (const key of ["polynomial", "initValue", "finalXOR"] as const) {
    ...
    if (typeof v === "string") {
      if (!HEX_PARAM_RE.test(v))
        errors.push({
          message: `${ctx}: checksumParams.${key} hex string "${v}" must match ^0x[0-9A-Fa-f]+$ (§8/§11.1).`,
        });
    } else if (typeof v === "number") {
      if (!Number.isInteger(v) || v < 0)
        errors.push({
          message: `${ctx}: checksumParams.${key} must be a non-negative integer or a 0x hex string (§8).`,
        });
      else if (!Number.isSafeInteger(v))
        errors.push({
          message: `${ctx}: checksumParams.${key} exceeds 2^53−1 and must be written as a ^0x[0-9A-Fa-f]+$ hex string to preserve precision (§8/§11.1).`,
        });
    } else {
      errors.push({
        message: `${ctx}: checksumParams.${key} must be a non-negative integer or a 0x hex string (§8).`,
  ...
  if (p.width !== undefined) {
    if (!Number.isInteger(p.width) || p.width < 1 || p.width > 64)
      errors.push({
        message: `${ctx}: checksumParams.width must be an integer in 1–64, got ${String(p.width)} (§8).`,
      });
  } else if (!typeHasDeclaredWidth) {
    errors.push({
      message: `${ctx}: checksumParams on a field whose type has no single declared bit width (e.g. bytes) requires an explicit width (§8/§11.1).`,
    });
  }
}
...
function validateSubfields(
  ...
  const isByteAlignedBits =
    t.kind === "bits" && Number.isInteger(t.n) && t.n % 8 === 0;
  if (!isInt && !isByteAlignedBits) {
    errors.push({
      message: `${ctx}: subfields are only allowed on an int field or a byte-aligned bits field (n a multiple of 8), §12/§11.1.`,
    });
    return;
  }
  ...
  subs.forEach((sf, i) => {
    ...
    for (const key of Object.keys(sf))
      if (!SUBFIELD_KEYS.has(key))
        errors.push({
          message: `${tag} has unknown key "${key}" (allowed: ${[...SUBFIELD_KEYS].join(", ")}) (§12).`,
        });
    if (typeof sf.id !== "string" || !ID_RE.test(sf.id))
      errors.push({
        message: `${tag}: id "${String(sf.id)}" must match ${ID_RE} (§12).`,
      });
    ...
    if (m === undefined)
      errors.push({
        message: `${tag}: mask must be a non-negative integer or a ^0x[0-9A-Fa-f]+$ hex string (§12).`,
      });
    else if (m >= limit)
      errors.push({
        message: `${tag}: mask ${String(sf.mask)} does not fit within the field's declared ${width}-bit width (§12/§11.1).`,
      });
    if (sf.level !== undefined && !NORM_LEVELS.has(sf.level as string))
      errors.push({
        message: `${tag}: invalid level "${String(sf.level)}" (must be must|should|may) (§12).`,
      });
    if (
      sf.category !== undefined &&
      !CATEGORY_TOKEN_SET.has(sf.category as string)
    )
      errors.push({
        message: `${tag}: category "${String(sf.category)}" is not one of the nine category tokens (§5.1).`,
      });
...
function validateChecksumCovers(field: Field, ctx: string, w: WalkCtx): void {
  ...
  if (!Array.isArray(covers)) {
    w.errors.push({
      message: `${ctx}: checksumCovers must be an array of field ids (§8).`,
    });
    return;
  }
  ...
  for (const id of covers) {
    if (typeof id !== "string" || id.length === 0) {
      w.errors.push({
        message: `${ctx}: checksumCovers entries must be non-empty field ids (§8).`,
      });
      continue;
    }
    ...
    if (head !== "" && prefixes !== undefined && prefixes.has(head)) {
      w.errors.push({
        message: `${ctx}: checksumCovers "${id}" is an import-qualified def name; use the instantiation id or its dotted leaf form (§1.3/§11.1).`,
      });
      continue;
    }
    if (known !== undefined && !known.has(id)) {
      w.errors.push({
        message: `${ctx}: checksumCovers "${id}" is not declared anywhere in the packet (§8/§11.1).`,
      });
    }
  }
}
...
function validateField(field: Field, ctx: string, w: WalkCtx): void {
  if (typeof field.id !== "string" || !ID_RE.test(field.id))
    w.errors.push({
      message: `${ctx}: field id "${String(field.id)}" must match ${ID_RE}.`,
    });
  if (typeof field.name !== "string")
    w.errors.push({
      message: `${ctx}: field "${field.id}" is missing a name.`,
    });
  if (!field.type) {
    w.errors.push({
      message: `${ctx}: field "${field.id}" is missing a type.`,
    });
    return;
  }
  ...
  if (field.category !== undefined && !CATEGORY_TOKEN_SET.has(field.category))
    w.errors.push({
      message: `${ctx}/${field.id}: category "${String(field.category)}" is not one of the nine category tokens (§5.1).`,
    });
  ...
  if (
    ...
    w.errors.push({
      message: `${ctx}/${field.id}: checksumParams cannot be used with the non-CRC algorithm "${field.checksumAlgorithm}" (§8/§11.1).`,
    });
  ...
  if (
    ...
    w.errors.push({
      message: `${ctx}/${field.id}: byteOrder must be 'BE' or 'LE'.`,
    });
  if (field.computedFrom !== undefined) {
    if (field.computedFrom.kind !== "wireSize")
      w.errors.push({
        message: `${ctx}/${field.id}: computedFrom must be a wireSize expression.`,
      });
    else
      validateExprPlacement(
        field.computedFrom,
        "computedFrom",
        `${ctx}/${field.id}`,
        w.errors,
  ...
  if (field.values !== undefined) {
    if (!Array.isArray(field.values)) {
      w.errors.push({
        message: `${ctx}/${field.id}: values must be an array (§5.3).`,
      });
    } else {
      field.values.forEach((ve, i) => {
        ...
        for (const key of Object.keys(ve)) {
          if (!VALUE_ENTRY_KEYS.has(key))
            w.errors.push({
              message: `${tag} has unknown key "${key}" (allowed: ${[...VALUE_ENTRY_KEYS].join(", ")}) (§5.3).`,
            });
        }
        ...
        const forms =
          (hasValue ? 1 : 0) + (hasRange ? 1 : 0) + (hasPattern ? 1 : 0);
        if (forms !== 1)
          w.errors.push({
            message: `${tag} must set exactly one of 'value', 'range', or 'pattern' (§5.3).`,
          });
        ...
        if (hasRange) {
          ...
          if (
            ...
            w.errors.push({
              message: `${tag}: range must be [min, max] integers with min ≤ max.`,
            });
        }
        if (
          hasPattern &&
          (typeof ve.pattern !== "string" || !PATTERN_RE.test(ve.pattern))
        )
          w.errors.push({
            message: `${tag}: pattern must be a non-empty string of 0, 1, or x/X.`,
          });
        ...
        if (ve.level !== undefined && !NORM_LEVELS.has(ve.level))
          w.errors.push({
            message: `${tag}: invalid level "${String(ve.level)}" (must be must|should|may).`,
          });
...
function withOpen(w: WalkCtx, id: string | undefined, body: () => void): void {
  ...
  try {
    ...
    if (added) w.pc.openIds.delete(id);
  }
}
...
function validateGroup(g: Group, ctx: string, w: WalkCtx): void {
  ...
  if (!Array.isArray(g.children))
    w.errors.push({
      message: `${ctx}: group "${g.id}" must have a children array.`,
    });
  else
    withOpen(w, g.id, () => {
...
function validateRepeat(r: Repeat, ctx: string, w: WalkCtx): void {
  ...
  if (r.count === "eos") {
    ...
    for (const key of Object.keys(r.count))
      if (key !== "until")
        w.errors.push({
          message: `${sub}: repeat count until-object accepts only the "until" key (got "${key}").`,
        });
    if (!isValidExpr(r.count.until))
      w.errors.push({
        message: `${sub}: repeat until has a malformed expression.`,
      });
    else
      validateExprPlacement(
        r.count.until,
    ...
    w.errors.push({
      message: `${sub}: repeat count has a malformed expression.`,
    });
  } else {
    validateExprPlacement(
      r.count,
...
function validateSwitch(s: Switch, ctx: string, w: WalkCtx): void {
  ...
  if (!isValidExpr(s.on))
    w.errors.push({
      message: `${sub}: switch has an invalid discriminator expression.`,
    });
  ...
  withOpen(w, s.id, () => {
    for (const [key, struct] of Object.entries(s.cases)) {
      if (!SWITCH_KEY_RE.test(key)) {
        w.errors.push({
          message: `${sub}: invalid switch case key "${key}" (use decimal, "lo-hi", "a,b,c", or "_").`,
        });
      } else {
        const range = /^(0|[1-9][0-9]*)-(0|[1-9][0-9]*)$/.exec(key);
        if (range && Number(range[1]) > Number(range[2]))
          w.errors.push({
            message: `${sub}: invalid range switch case key "${key}" (lo must be <= hi).`,
          });
      }
...
function validateEncrypted(e: Encrypted, ctx: string, w: WalkCtx): void {
  ...
  if (!e.plaintext || !Array.isArray(e.plaintext.fields))
    w.errors.push({
      message: `${sub}: encrypted container must have a plaintext struct.`,
    });
  ...
  if (
    Array.isArray(e.headerProtected) &&
    e.plaintext &&
    Array.isArray(e.plaintext.fields)
  ) {
    ...
    for (const hp of e.headerProtected) {
      if (typeof hp !== "string" || !resolvable.has(hp))
        w.errors.push({
          message: `${sub}: headerProtected id "${String(hp)}" resolves to neither a plaintext field of this encrypted container nor a field declared earlier in the same body (§5/§11.1).`,
        });
    }
  }
...
function validateOptional(o: Optional, ctx: string, w: WalkCtx): void {
  ...
  if (!isValidExpr(o.when))
    w.errors.push({
      message: `${ctx}: optional has an invalid 'when' expression.`,
    });
...
function validateContainerCtx(c: Container, ctx: string, w: WalkCtx): void {
  ...
  if (w.pc) {
    ...
    if (w.pc.declaredExprIds !== undefined)
      collectSubtreeDeclaredIds(
        c,
        w.defs,
        "",
        w.pc.declaredExprIds,
        new Set(),
        0,
      );
  }
  ...
  switch (c.kind) {
    ...
    case "virtual":
      if (typeof c.id !== "string" || !ID_RE.test(c.id))
        w.errors.push({
          message: `${ctx}: virtual id "${String(c.id)}" must match ${ID_RE}.`,
        });
      if (w.inDef)
        w.errors.push({
          message: `${ctx}: virtual field "${c.id}" is forbidden inside a defs struct.`,
        });
      if (!isValidExpr(c.expr))
        w.errors.push({
          message: `${ctx}: virtual "${c.id}" has a malformed expr.`,
        });
    ...
    case "align":
      if (
        ...
        w.errors.push({
          message: `${ctx}: align 'to' must be a power of 2 and a multiple of 8, got ${c.to}.`,
        });
      if (
        c.fill !== undefined &&
        (!Number.isInteger(c.fill) || c.fill < 0 || c.fill > 255)
      )
        w.errors.push({
          message: `${ctx}: align 'fill' must be 0–255, got ${c.fill}.`,
        });
      return;
    case "ref":
      if (typeof c.ref !== "string" || c.ref.length === 0)
        ...
        w.errors.push({
          message: `${ctx}: ref target "${c.ref}" not found in defs.`,
        });
      if (typeof c.id !== "string" || !ID_RE.test(c.id))
        w.errors.push({
          message: `${ctx}: ref container id "${String(c.id)}" must match ${ID_RE}.`,
        });
      return;
  }
}
...
function validateStruct(s: Struct, ctx: string, w: WalkCtx): void {
  ...
  if (!Array.isArray(s.fields)) {
    w.errors.push({
      message: `${ctx}/${s.id ?? "?"}: struct must have a fields array.`,
    });
    return;
  }
...
function collectSubtreeDeclaredIds(
  ...
  if (isField(c)) {
    if (typeof c.id === "string") add(c.id);
    return;
  }
  switch (c.kind) {
    case "virtual":
...
function detectRefCycles(
  ...
  const dfs = (name: string, stack: string[]): void => {
    ...
    if (visiting.has(name)) {
      errors.push({
        message: `defs: circular reference through non-recursive def "${name}" (${[...stack, name].join(" → ")}).`,
      });
      return;
    }
    visiting.add(name);
...
export function validatePacket(packet: Packet): ValidationError[] {
  ...
  if (rowBits !== undefined && (!Number.isInteger(rowBits) || rowBits <= 0))
    errors.push({
      message: `rowBits must be a positive integer, got ${String(rowBits)}.`,
    });
  if (!Array.isArray(packet.body))
    errors.push({ message: "Packet must have a body array." });
  if (
    packet.byteOrder !== undefined &&
    packet.byteOrder !== "BE" &&
    packet.byteOrder !== "LE"
  )
    errors.push({
      message: `byteOrder must be 'BE' or 'LE', got "${String(packet.byteOrder)}".`,
    });
  if (packet.version !== undefined && !/^\d+\.\d+$/.test(packet.version))
    errors.push({
      message: `version must be "MAJOR.MINOR", got "${packet.version}".`,
    });
  ...
  if (packet.imports) {
    ...
    for (const imp of packet.imports) {
      ...
      if (typeof imp.as !== "string" || !/^[a-zA-Z][a-zA-Z0-9_]*$/.test(imp.as))
        errors.push({
          message: `imports: 'as' prefix "${String(imp.as)}" must match [a-zA-Z][a-zA-Z0-9_]*.`,
        });
        ...
        if (localDefNames.has(imp.as))
          errors.push({
            message: `imports: 'as' prefix "${imp.as}" collides with a local defs key of the same name; a dotted reference such as "${imp.as}.x" would be ambiguous (§1.2/§6).`,
          });
        seen.add(imp.as);
      }
    }
  ...
  for (const [name, def] of Object.entries(defs)) {
    ...
    if (!Array.isArray(def.fields)) {
      errors.push({
        message: `defs/${name}: struct must have a fields array.`,
      });
      continue;
    }
  ...
  for (const [i, con] of (packet.constraints ?? []).entries()) {
    ...
    for (const key of Object.keys(con)) {
      if (!CONSTRAINT_KEYS.has(key))
        errors.push({
          message: `constraints[${i}] has unknown key "${key}" (allowed: ${[...CONSTRAINT_KEYS].join(", ")}) (§9).`,
        });
    }
    ...
    if (con.level !== undefined && !NORM_LEVELS.has(con.level))
      errors.push({
        message: `constraints[${i}]: invalid level "${String(con.level)}" (must be must|should|may).`,
      });
  }
  return errors;
}
