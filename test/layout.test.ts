...
describe("resolveLayout — value dictionary reaches LayoutField (§5.3)", () => {
  it("carries field.values/meta onto the flat LayoutField", () => {
    const pkt: Packet = {
      ...
      rowBits: 32,
      body: [{
        id: "proto", name: "Protocol", type: { kind: "int", bits: 8 },
        meta: { rfc: 790 },
        values: [{ value: 6, name: "TCP" }, { value: 17, name: "UDP" }],
      }],
    };
    const { cells } = resolveLayout(pkt);
    const f = cells.find((c) => c.field.id === "proto")!.field;
    expect(f.values).toEqual([{ value: 6, name: "TCP" }, { value: 17, name: "UDP" }]);
  ...
  it("carries the encrypted region's meta onto the wire-view blob LayoutField (§5.4)", () => {
    const pkt: Packet = {
      ...
      body: [{
        ...
        plaintext: {
          id: "pt",
          fields: [{ id: "inE", name: "InE", type: { kind: "int", bits: 24 } }],
        },
      }],
    };
    const { cells } = resolveLayout(pkt, { viewMode: "wire" });
    const f = cells.find((c) => c.field.id === "payload")!.field;
    expect(f.meta).toEqual({ rfc: { defined: 9001, updates: [9999] }, section: "4" });
  });
});
...
describe("resolveLayout — group RFC provenance reaches the parent LayoutField (§5.4)", () => {
  it("surfaces Group.meta on the collapsed parent field", () => {
    ...
    expect(parent.meta).toEqual({ rfc: { defined: 791, updates: [2474, 3168] }, section: "1.4" });
  });
  ...
  it("surfaces group meta on a single-child group (flat branch fallback)", () => {
    const pkt: Packet = {
      ...
      rowBits: 8,
      body: [{
        kind: "group", id: "solo", name: "Solo",
        meta: { rfc: 999 },
        children: [{ id: "only", name: "Only", type: { kind: "int", bits: 8 } }],
      }],
    };
    const { cells } = resolveLayout(pkt);
    expect(cells.find((c) => c.field.id === "only")!.field.meta).toEqual({ rfc: 999 });
  });
  ...
  it("prefers field meta over group meta when both are present", () => {
    const pkt: Packet = {
      ...
      rowBits: 8,
      body: [{
        kind: "group", id: "g", name: "G", meta: { rfc: 100 },
        children: [{ id: "f", name: "F", type: { kind: "int", bits: 8 }, meta: { rfc: 200 } }],
      }],
    };
    const { cells } = resolveLayout(pkt);
    expect(cells.find((c) => c.field.id === "f")!.field.meta).toEqual({ rfc: 200 });
  });
  ...
  it("a virtual between two group fields does not split the collapse (§5.4)", () => {
    const pkt: Packet = {
      ...
      body: [{
        ...
        children: [
          { id: "ga", name: "GA", type: { kind: "int", bits: 4 }, meta: { rfc: 100 } },
  ...
  it("a group whose only sized member sits next to a virtual still gets the flat groupMeta fallback", () => {
    const pkt: Packet = {
      ...
      rowBits: 8,
      body: [{
        kind: "group", id: "solo", name: "Solo", meta: { rfc: 999 },
        children: [
...
describe("resolveLayout — subfield id stripping under nested repeats", () => {
  ...
  it("strips a nested-repeat suffix (#N_M) the same way", () => {
    const pkt: Packet = {
      ...
      rowBits: 8,
      body: [{
        kind: "repeat", id: "outer", count: { kind: "lit", value: 1 },
        element: {
          id: "oel",
          fields: [{
            kind: "repeat", id: "inner", count: { kind: "lit", value: 2 },
    ...
    for (const c of parents) {
      ...
      expect(c.subCells!.map((s) => s.id)).toEqual([`${c.field.id}:a`, `${c.field.id}:b`]);
    }
  });
});
